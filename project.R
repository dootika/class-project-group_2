...
links_generator <- function (ulimit)
{
  links <- NULL
  for (i in seq(0,ulimit,by=50)) #will change upper limit to 25350
  {
...
# Setting upper limit
anime_number <- 10 # change this to total number of animes you want to have in df
links_generator(ulimit=0)
...
for (i in 1:length(links))
{
  ...
  #picture.link <- html %>% html_elements (".lazyload a") %>% html_attr("src")
...
uids <- sapply (1:anime_number, function (i) strsplit(substring(anime.links,31), "/")[[i]][1])
...
theme_list <- theme_matcher("ActionAction","High Stakes Game")
theme_list <- theme_matcher("High Stakes GameHigh Stakes Game","Iyashikei")
...
anime_df <- data.frame("Unique IDs"=uids, "Ranking"=ranked_cleaned, "Anime Title"=anime.names,
