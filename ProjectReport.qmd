...
MainData <- read.csv("C:/Users/Swapnonil/Downloads/Finaldata.csv")
...
ggplot(genres, aes(x="", y=Freq, fill = Var1))+ 
      ...
      labs(fill = "Genre")
```
>>>>>>> 524d02bdf16e50e74e77f047cf022ddebd400e52
...
Now let us concentrate in a comparative analysis between the acceptance of the Anime Shows according to the age groups. Here is a pie chart and some box plots, where the preferences of the genre for the 17+ audience.
...
From the pie chart and the box plot, we can see that for the Anime shows with manga as their source, the action genre is more popular than the rest. Comedy, fantasy, horror and supernatural genres appeals lesser to the 17+ audience.

Another pie chart is here where the preferences of the juvenile audience is shown.

```{r, echo=FALSE}
library(ggplot2)
genres <- as.data.frame(table(MainData$Genre[which((MainData$Source=="Manga") & (MainData$Rating=="PG - Children"))]))
genres <- genres[order(genres$Freq,decreasing = TRUE),]
genres <- genres[1:5,]
ggplot(genres, aes(x="", y=Freq, fill = Var1))+ 
      geom_bar(stat="identity", width=1, color = "white")+ 
...
For the juvenile audience, the scenario changes. The comedy and the adventure genre appeals the most, which is pretty much evident.
